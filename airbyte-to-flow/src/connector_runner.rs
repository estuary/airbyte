...
use crate::libs::stream::stream_lines;
...
pub async fn run_airbyte_source_connector(
    ...
    // Create a persistent line stream that we can read from multiple times
    let mut line_stream = Box::pin(stream_lines(in_stream));

    // Process multiple request-response cycles
    loop {
        let writer = Arc::clone(&response_write);
        // Read the next request from the input stream
        let (op, request) = match airbyte_interceptor.next_request_from_lines(&mut line_stream).await {
            Ok(result) => result,
            Err(Error::EmptyStream) => {
                tracing::debug!("atf: input stream closed, exiting");
                break;
            }
            Err(e) => return Err(e),
        };
        ...
        let adapted_request_stream = airbyte_interceptor.adapt_request_stream(&op, request)?;
        ...
        let exit_status_task = async move {
            ...
            if exit_status_result.is_ok() && op == Operation::Capture {
        ...
        let result = tokio::select! {
            ...
            Err(_) = ping_timeout_task => {
                ...
                child_arc.lock().await.kill().await?;
                Ok(())
            }
        };

        // Handle errors and check if we should continue
        match result {
            Err(e) => return Err(e),
            Ok(()) => {
                // For capture operations, run the interval check and then exit
                if op == Operation::Capture {
                    run_interval.await?;
                    break;
                }
                // For other operations (spec, discover, validate, apply), continue to next request
            }
        }
    }
